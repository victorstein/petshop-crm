...
export const collectibleCardRewardGeneration = onRequest(
  (request, response) => {
    logger.info('collectibleCardRewardGeneration', { structuredData: true })
    response.send(
      'TODO: Implement Gemini to send image prompt and collectible card description to IGS'
    )
  }
)

export const collectibleCardRewardPersistence = onRequest(
  (request, response) => {
...
export const pixarProfileImageGeneration = onRequest((request, response) => {
  ...
  response.send(
    'TODO: Trigger to call IGS for the Pixar versions of the profile image, using Gemini and Replicate'
  )
})
